...
public final class MailSendService {
    ...
    private func appendToSentFolder(draft: MailDraft, accountId: UUID) async {
        ...
        logger.info(.SEND, accountId: accountId, "📤 Sent folder: \(sentFolder)")
        ...
        logger.info(.SEND, accountId: accountId, "📤 Account loaded: \(account.recvHost):\(account.recvPort)")
        ...
        do {
            ...
            defer { conn.close() }
            logger.info(.SEND, accountId: accountId, "📤 Connection opened")

            let commands = IMAPCommands()

            // Server-Greeting empfangen (wichtig vor allen anderen Kommandos!)
            logger.info(.SEND, accountId: accountId, "📤 Waiting for greeting...")
            _ = try await commands.greeting(conn)
            logger.info(.SEND, accountId: accountId, "📤 Greeting received")

            // STARTTLS falls nötig (vor Login)
            if account.recvEncryption == .startTLS {
                logger.info(.SEND, accountId: accountId, "📤 Starting TLS...")
                try await commands.startTLS(conn)
                logger.info(.SEND, accountId: accountId, "📤 TLS started")
            }

            // Login
            logger.info(.SEND, accountId: accountId, "📤 Logging in as \(account.recvUsername)...")
