...
struct LogsView: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
                // Header (aligned like in SprechenView)
                VStack(spacing: 6) {
                    HStack {
                        Text("logs.header.title")
                            .font(.title)
                            .bold()
                            .frame(maxWidth: .infinity, alignment: .center)
                    }
                    Text("logs.header.subtitle")
                        .font(.subheadline)
                ...
                .overlay(alignment: .topTrailing) {
                    ...
                    .accessibilityLabel(Text("logs.action.config"))
                }
                .padding(.top, 20) // leicht nach unten wie in SprechenView
                .padding(.bottom, 4)
